...
func (s *AuthService) Authenticate(loginData UserLogin) (*models.User, error) {
	...
	if err := db.DB.Preload("Roles").Preload("Pharmacies").Preload("Employee").Where("email = ?", loginData.Email).First(&user).Error; err != nil {
...
func (s *AuthService) RefreshTokens(tokenString string) (*TokenResponse, error) {
	...
	if err != nil {
		log.Printf("[RefreshTokens] VerifyToken failed: %v\n", err)
		return nil, errors.New("invalid or expired refresh token")
	}

	if claims.Type != "refresh" {
		log.Printf("[RefreshTokens] Invalid token type: %s\n", claims.Type)
		return nil, errors.New("invalid token type")
	}

	var user models.User
	if err := db.DB.Preload("Roles").Preload("Pharmacies").Preload("Employee").Where("id = ?", claims.Subject).First(&user).Error; err != nil {
		log.Printf("[RefreshTokens] User not found for id %v. Error: %v\n", claims.Subject, err)
		return nil, errors.New("user not found")
	}

	if user.TokenVersion != claims.Version {
		log.Printf("[RefreshTokens] Token revoked. User version: %d, Claims version: %d\n", user.TokenVersion, claims.Version)
...
func (s *AuthService) MapUserToResponse(user *models.User, tiers map[int]repositories.PharmacyTier) *UserResponse {
	...
	if user.Employee != nil {
		...
		empRole = user.Employee.Role // e.g. 'preparation', 'control', 'gomla', etc.
	}

	var roles []string
	for _, r := range user.Roles {
		roles = append(roles, r.Name) // Only: admin, pharmacist, employee
	}
	...
	return &UserResponse{
		...
		CanAccessEmployee: empID != nil, // true if linked to an employee record
		EmployeeID:        empID,
...
func (s *AuthService) Register(userIn UserCreate) (*models.User, error) {
	...
	// Add default pharmacist role
	var defaultRole models.Role
	if err := db.DB.Where("name = ?", "pharmacist").FirstOrCreate(&defaultRole, models.Role{Name: "pharmacist"}).Error; err == nil {
		db.DB.Model(&user).Association("Roles").Append(&defaultRole)
		user.Roles = append(user.Roles, defaultRole) // Update memory
	}
...
func (s *AuthService) UpdateProfile(user *models.User, updateData UserProfileUpdate) (*UserResponse, error) {
	if updateData.Email != "" && updateData.Email != user.Email {
		var existing models.User
		if err := db.DB.Where("email = ?", updateData.Email).First(&existing).Error; err == nil {
	...
	if updateData.ManagerName != "" {
		user.ManagerName = updateData.ManagerName
	}
	if updateData.ManagerPhone != "" {
		user.ManagerPhone = updateData.ManagerPhone
	}
...
func (s *AuthService) UpdateAvatarUrl(user *models.User, avatarUrl string) error {
...
func (s *AuthService) LoginOrRegisterGoogle(info map[string]interface{}) (*models.User, error) {
	...
	if err := db.DB.Preload("User").Preload("User.Pharmacies").Preload("User.Roles").Preload("User.Employee").Where("google_id = ?", googleID).First(&googleUser).Error; err == nil {
	...
	var user models.User
	if err := db.DB.Preload("Roles").Preload("Pharmacies").Preload("Employee").Where("email = ?", email).First(&user).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// Register new user
			user = models.User{
				Email:           email,
