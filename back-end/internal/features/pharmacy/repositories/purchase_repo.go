...
func (r *PurchaseRepository) GetPurchases(pharmaCodes []int, limit int, skip int, sort string) (*sql.Rows, error) {
	...
	finalQuery := fmt.Sprintf(`
		SELECT FIRST ? SKIP ?
			ID, DATE_D, TIME_T, TOTAL, SRC, USR, ACC, CUS_NAME
		FROM (
			SELECT H.INVOICES_H_ID as ID, H.DATE_D, H.TIME_T, CAST(H.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 'H' as SRC, H.USERS_NAME as USR, A.ACCOUNT_NAME as ACC, H.CUS_NAME as CUS_NAME
			FROM INVOICES_H H JOIN ACCOUNTS A ON H.ACCOUNT_ID = A.ACCOUNT_ID
			WHERE H.ACCOUNT_ID IN (%s) AND H.TOTAL_TOTAL <> 0
			UNION ALL
			SELECT H.INVOICES_HH_ID as ID, H.DATE_D, H.TIME_T, CAST(H.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 'HH' as SRC, H.USERS_NAME as USR, A.ACCOUNT_NAME as ACC, H.CUS_NAME as CUS_NAME
...
func (r *PurchaseRepository) GetReturns(pharmaCodes []int, limit int, skip int, sort string) (*sql.Rows, error) {
	...
	query := fmt.Sprintf(`
		SELECT FIRST ? SKIP ?
			ID, DATE_D, TIME_T, TOTAL, SRC, USR, ACC, CUS_NAME
		FROM (
			SELECT H.INVOICES_R_H_ID as ID, H.DATE_D, H.TIME_T, CAST(H.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 'R' as SRC, H.USERS_NAME as USR, A.ACCOUNT_NAME as ACC, CAST('' AS VARCHAR(255)) as CUS_NAME
			FROM INVOICES_R_H H JOIN ACCOUNTS A ON H.ACCOUNT_ID = A.ACCOUNT_ID
			WHERE H.ACCOUNT_ID IN (%s) AND H.TOTAL_TOTAL <> 0
			UNION ALL
			SELECT H.INVOICES_RR_H_ID as ID, H.DATE_D, H.TIME_T, CAST(H.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 'RR' as SRC, H.USERS_NAME as USR, A.ACCOUNT_NAME as ACC, CAST('' AS VARCHAR(255)) as CUS_NAME
...
func (r *PurchaseRepository) GetPurchaseDetailsBatch(ids []string) (*sql.Rows, error) {
	if len(ids) == 0 {
	...
	var numIDs []string
	for _, id := range ids {
		// Only include valid numeric IDs
	...
	if len(numIDs) == 0 {
		return nil, fmt.Errorf("no valid numeric ids provided")
	}

	idStr := strings.Join(numIDs, ",")

	query := fmt.Sprintf(`
		SELECT
			'H_' || I.INVOICES_H_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(I.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(I.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(I.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
			'' as BN, '' as EXP
		FROM INVOICES_D I
		LEFT JOIN PRODUCTS P ON I.PROD_ID = P.PROD_ID
		WHERE I.INVOICES_H_ID IN (%s)
		UNION ALL
		SELECT 
			'HH_' || I.INVOICES_HH_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(I.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(I.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(I.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
		...
		FROM INVOICES_DD I
		LEFT JOIN PRODUCTS P ON I.PROD_ID = P.PROD_ID
		WHERE I.INVOICES_HH_ID IN (%s)
		UNION ALL
		SELECT 
			'O_' || O.ORDER_H_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(O.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(O.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(O.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
			'' as BN, '' as EXP
		FROM ORDER_D O
		LEFT JOIN PRODUCTS P ON O.PROD_ID = P.PROD_ID
		WHERE O.ORDER_H_ID IN (%s)
		UNION ALL
		SELECT 
			'R_' || R.INVOICES_R_H_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(R.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(R.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(R.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
			'' as BN, '' as EXP
		FROM INVOICES_R_D R
		LEFT JOIN PRODUCTS P ON R.PROD_ID = P.PROD_ID
		WHERE R.INVOICES_R_H_ID IN (%s)
		UNION ALL
		SELECT 
			'RR_' || RR.INVOICES_RR_H_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(RR.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(RR.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(RR.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
			'' as BN, '' as EXP
		FROM INVOICES_RR_D RR
		LEFT JOIN PRODUCTS P ON RR.PROD_ID = P.PROD_ID
		WHERE RR.INVOICES_RR_H_ID IN (%s)
		UNION ALL
		SELECT 
			'OR_' || ORR.ORDER_R_H_ID as H_ID,
			P.PROD_NAME as NAME, 
			CAST(ORR.TOTAL_QTY_ALL AS DOUBLE PRECISION) as QTY, 
			CAST(ORR.CONSUMER AS DOUBLE PRECISION) as PRICE, 
			CAST(ORR.TOTAL_TOTAL AS DOUBLE PRECISION) as TOTAL, 
			'' as BN, '' as EXP
		FROM ORDER_R_D ORR
		LEFT JOIN PRODUCTS P ON ORR.PROD_ID = P.PROD_ID
		WHERE ORR.ORDER_R_H_ID IN (%s)
	`, idStr, idStr, idStr, idStr, idStr, idStr)

	return db.FB.Query(query)
}
