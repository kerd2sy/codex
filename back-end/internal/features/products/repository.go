...
func encodeToWin1256(s string) interface{} {
	win1256Str, err := charmap.Windows1256.NewEncoder().String(s)
	if err == nil {
		return win1256Str
	}
	return s
	...
	if search != "" {
		...
		for _, kw := range keywords {
			if len(kw) >= 2 {
				...
				exactParams = append(exactParams, encodeToWin1256(kw))
				
				if len([]rune(kw)) >= 4 {
					strippedKw := strings.ReplaceAll(kw, "ي", "")
					strippedKw = strings.ReplaceAll(strippedKw, "ا", "")
					strippedKw = strings.ReplaceAll(strippedKw, "و", "")
					
					if len([]rune(strippedKw)) >= 3 {
						fuzzyConds = append(fuzzyConds, "REPLACE(REPLACE(REPLACE(P.PROD_NAME, ?, ''), ?, ''), ?, '') CONTAINING ?")
						fuzzyParams = append(fuzzyParams, encodeToWin1256("ي"), encodeToWin1256("ا"), encodeToWin1256("و"), encodeToWin1256(strippedKw))
					}
				}
			}
		}
		...
		encSearchClean := encodeToWin1256(searchClean)
		params = append(params, encSearchClean, encSearchClean, encSearchClean, encSearchClean)

		if matched, _ := regexp.MatchString(`^\d+$`, searchClean); matched {
			val, _ := strconv.Atoi(searchClean)
			if val <= 2147483647 {
				otherConds = append(otherConds, "P.PROD_ID = ?")
...
func (r *ProductRepository) GetRecent(limit int, kind int) ([]map[string]interface{}, error) {
	...
	rows, err := r.fb.Query(query, limit, dateStr)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
