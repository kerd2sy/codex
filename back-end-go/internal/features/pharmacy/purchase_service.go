...
func (s *PurchaseService) mapRows(rows *sql.Rows) ([]map[string]interface{}, error) {
	...
	for rows.Next() {
		var (
			id, source, writer, pharmName sql.NullString
			date, timeRaw                 sql.NullString
			total                         sql.NullFloat64
		)
		rows.Scan(&id, &date, &timeRaw, &total, &source, &writer, &pharmName)
...
func (s *PurchaseService) getInvoiceDetailGeneric(idStr string, mode string) (map[string]interface{}, error) {
	...
	err = headerRow.Scan(&invID, &date, &timeRaw, &total, &writer, &pharmName, &accountID, &bacet, &kartona, &freeze)
	if err != nil {
		return nil, err
	}
